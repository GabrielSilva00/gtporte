...
const COR:Record<string,{icon:typeof CheckCircle;cls:string;label:string}>={aprovado:{icon:CheckCircle,cls:'text-ok',label:'Aprovado'},pendente:{icon:Clock,cls:'text-warn',label:'Em analise'},rejeitado:{icon:XCircle,cls:'text-err',label:'Rejeitado'}}
    ...
    <div className="space-y-2">{TIPOS_DOC.map((tipo,i)=>{
      ...
      return <div key={tipo} className="card anim-in" style={{animationDelay:`${i*40}ms`}}>
        ...
        {podeEnviar&&<div className="mt-3">
          ...
          <button onClick={()=>refs.current[tipo]?.click()} disabled={!!busy} className={`w-full flex items-center justify-center gap-2 rounded-xl py-2.5 text-xs font-semibold transition-all ${doc?'bg-raised/60 text-muted':'bg-brand-600/20 text-brand-500'}`}>
