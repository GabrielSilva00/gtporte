...
interface Msg { id:string; assunto:string; corpo:string; criado_em:string }

export function Feedback(){
  const [msgs,setMsgs]=useState<Msg[]>([]);const [loading,setLoading]=useState(true)
  const [show,setShow]=useState(false);const [assunto,setAssunto]=useState('');const [corpo,setCorpo]=useState('');const [busy,setBusy]=useState(false)
  const [aberta,setAberta]=useState<string|null>(null)

  const refresh=useCallback(async()=>{
    const{data:{user}}=await supabase.auth.getUser();if(!user){setLoading(false);return}
    const{data}=await supabase.from('mensagem').select('id,assunto,corpo,criado_em,remetente:remetente_id(nome)').or(`remetente_id.eq.${user.id},destinatario_id.eq.${user.id}`).order('criado_em',{ascending:false}).limit(30)
    setMsgs(((data||[]) as any) as Msg[]);setLoading(false)
  },[])
  useEffect(()=>{refresh()},[refresh])

  const enviar=async()=>{if(!assunto.trim()||!corpo.trim())return;setBusy(true);try{
    const{error}=await supabase.from('mensagem').insert({assunto:assunto.trim(),corpo:corpo.trim()});if(error)throw new Error(erroMsg(error))
    toast('Mensagem enviada!');setAssunto('');setCorpo('');setShow(false);await refresh()
  ...
  return <div className="px-4 pb-24 pt-4 space-y-4">
    <div className="flex items-center justify-between"><h2 className="text-lg font-bold">Mensagens</h2>
      <button onClick={()=>setShow(true)} className="flex items-center gap-1.5 rounded-xl bg-brand-600 px-4 py-2.5 text-xs font-bold text-white"><Plus className="h-4 w-4"/>Nova</button></div>
    {show&&<div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm" onClick={()=>setShow(false)}>
      <div className="w-full max-w-lg rounded-t-3xl bg-navy-800 p-6 pb-10 anim-in" onClick={e=>e.stopPropagation()}>
        ...
        <button onClick={enviar} disabled={busy} className="btn-primary mt-4 flex items-center justify-center gap-2">{busy?<Spinner/>:<><Send className="h-4 w-4"/>Enviar</>}</button>
      </div></div>}
    {loading?<div className="flex justify-center py-12"><Spinner/></div>:msgs.length===0?
      ...
      <div className="space-y-2">{msgs.map((m,i)=><button key={m.id} onClick={()=>setAberta(aberta===m.id?null:m.id)} className="card w-full text-left anim-in" style={{animationDelay:`${i*30}ms`}}>
        <p className="text-sm font-semibold truncate">{m.assunto}</p>
        <p className="text-[10px] text-white/30 mt-0.5">{'Voce'} \u00B7 {fmt(m.criado_em)}</p>
